...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['F', 0.5], //From 40% to 49%
		['E', 0.7], //From 50% to 59%
		['D', 0.75], //From 60% to 68%
		['C', 0.8], //69%
		['B', 0.85], //From 70% to 79%
		['A', 0.9], //From 80% to 89%
		['S', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		if(ClientPrefs.timeBarType == 'Song Name')
		{
			...
			timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 20, 400, "", 32);
			timeTxt.setFormat(Paths.font("pm-full.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}
		...
		timeBarBG = new AttachedSprite('healthBarOld'); 
		if(ClientPrefs.coolhealthbar) timeBarBG = new AttachedSprite('healthBar');
		...
		if(ClientPrefs.coolhealthbar) healthBarBG = new AttachedSprite('healthBar');
		...
		judgementCounter.setFormat(Paths.font("pm-full.ttf"), 12, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("pm-full.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(ClientPrefs.coolhealthbar) healthBarOV.cameras = [camHUD];
	...
	//	timeBarBG.cameras = [camHUD];
		...
		if(isPixelStage && ClientPrefs.shaders) {
			...
			addShaderToCamera('hud', new VhsEffect(0.4, 0));
			//addShaderToCamera('camHUD', new VCRDistortionEffect(0.1, false, true, false));
		}

		if(curStage == 'devilsg' && ClientPrefs.shaders) {
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		judgementCounter.setFormat(Paths.font("pm-full.ttf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var animToPlay:String = '';
						switch (Math.abs(note.noteData))
						{
							case 0:
								animToPlay = 'singLEFT';
								if(ClientPrefs.camMove) dadCamFollowY = 0;
								if(ClientPrefs.camMove)	dadCamFollowX = -25;
							case 1:
								animToPlay = 'singDOWN';
								if(ClientPrefs.camMove) dadCamFollowY = 25;
								if(ClientPrefs.camMove)	dadCamFollowX = 0;
							case 2:
								animToPlay = 'singUP';
								if(ClientPrefs.camMove) dadCamFollowY = -25;
								if(ClientPrefs.camMove)	dadCamFollowX = 0;
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'ugh')
			{
				...
				switch(curBeat)
				{
					...
					case 15:
					FlxTween.tween(FlxG.camera, {zoom: 1.1}, 0.3, {
						ease: FlxEase.quadInOut
					});
					case 111:
					FlxTween.tween(FlxG.camera, {zoom: 1.1}, 0.3, {
		...
		if (curSong.toLowerCase() == 'guns' && curBeat >= 64 && curBeat < 124 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.03;
			}
		if (curSong.toLowerCase() == 'guns' && curBeat >= 128 && curBeat < 160 && camZooming && FlxG.camera.zoom < 1.35)
				{
					if (curBeat % 2 == 0)
						{
							FlxG.camera.zoom += 0.015;
		...
		if (curSong.toLowerCase() == 'guns' && curBeat >= 160 && curBeat < 224 && camZooming && FlxG.camera.zoom < 1.35)
					{
						FlxG.camera.zoom += 0.015;
						camHUD.zoom += 0.03;
					}
		if (curSong.toLowerCase() == 'guns' && curBeat >= 288 && curBeat < 352 && camZooming && FlxG.camera.zoom < 1.35)
						{
							FlxG.camera.zoom += 0.030;
							camHUD.zoom += 0.06;
						}
		if (curSong.toLowerCase() == 'guns' && curBeat >= 352 && curBeat < 384 && camZooming && FlxG.camera.zoom < 1.35)
							{
								if (curBeat % 2 == 0)
									{
										FlxG.camera.zoom += 0.015;
		...
		if (curSong.toLowerCase() == 'guns' && curBeat >= 384 && curBeat < 416 && camZooming && FlxG.camera.zoom < 1.35)
								{
									FlxG.camera.zoom += 0.015;
									camHUD.zoom += 0.03;
								}
